...
static void setup_for_endstop_move() {
  ...
  static float probe_pt(float x, float y, float z_before, ProbeAction probe_action = ProbeDeployAndStow, int verbose_level = 1) {
    ...
    void raise_z_for_servo() {
       ...
       */
      z_dest += axis_homed[Z_AXIS] ? zprobe_zoffset * _home_dir[Z_AXIS] : zpos;
  ...
  feedrate = 0.0;
    ...
    if (_home_dir[Z_AXIS] > 0 )  // If homing away from BED do Z first
      ...
      if (home_all_axis || homeZ) {
		...
		{
    #if defined(MIN_Z_HEIGHT_FOR_HOMING) && MIN_Z_HEIGHT_FOR_HOMING > 0
...
void calcReverse(const float x,const  float y,const float l,const float L,const float l2,const float L2,const float dhalf,float& q11,float& q22)
{
  ...
  SCARA_theta = ( atan2(x,y)-atan2(SCARA_K2, SCARA_K1) ) ;//* -1;
  SCARA_psi   =   atan2(SCARA_S2,SCARA_C2);
  
  q11 = SCARA_theta;
  q22 = SCARA_psi;//(SCARA_theta + SCARA_psi);
